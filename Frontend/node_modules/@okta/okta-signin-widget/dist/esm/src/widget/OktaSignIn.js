...
class OktaSignIn {
  constructor(options) {
    ...
    if (options.useInteractionCodeFlow && this.authClient.isPKCE() === false) {
      throw new Errors.ConfigError('The "useInteractionCodeFlow" option requires PKCE to be enabled on the authClient.');
  ...
  showSignInToGetTokens(options) {
    ...
    if (authClient.isAuthorizationCodeFlow() && !authClient.isPKCE()) {
      throw new Errors.ConfigError('"showSignInToGetTokens()" should not be used for authorization_code flow. ' + 'Use "showSignInAndRedirect()" instead');
    }
